...
import binary_search.signed_traditional;
import binary_search.unsigned_traditional;
import binary_search.alternative;
import binary_search.range;
...
public:
  ...
  [[nodiscard]] std::optional<index_t> constexpr impl(
      std::span<data_t const> const data,
      data_t const v) const noexcept {
    return (((data.size() > index_t(0)) &&
             std::binary_search(data.begin(), data.end(), v)))
...
public:
  ...
  template <class BS, char const *NAME>
  inline void testcase_indexless(benchmark::State const &state) noexcept {
    (void)state;
...
template <std::size_t N> class StaticFixtureBase : public benchmark::Fixture {
public:
  void SetUp(::benchmark::State &state) {
    (void)state;
...
extern "C" bool dynamic_binary_search_power(size_t const N,
                                            int32_t const data[],
                                            int32_t const value);
...
public:
  ...
  inline void search_first(benchmark::State const &state) {
    (void)state;
    BS bs{};
    bool const result = dynamic_binary_search_power(
        bench_.size(), bench_.data(), bench_.front());
    assert(result);
    (void)result;
  }
  ...
  inline void search_last(benchmark::State const &state) {
    ...
    BS bs{};
    bool const result = dynamic_binary_search_power(
        bench_.size(), bench_.data(), bench_.back());
    assert(result);
    (void)result;
  }
  ...
  inline void fail_first(benchmark::State const &state) {
    ...
    BS bs{};
    bool const result = dynamic_binary_search_power(
        bench_.size(), bench_.data(), bench_.front() - 1);
    assert(!result);
    (void)result;
  }
  ...
  inline void fail_last(benchmark::State const &state) {
    ...
    BS bs{};
    bool const result = dynamic_binary_search_power(
        bench_.size(), bench_.data(), bench_.back() + 1);
    assert(!result);
    (void)result;
  }
...
#define STATIC_DEFINE_FIXTURE(NAME, N)                                         \
  class STATIC_FIXTURE_NAME(NAME) : public StaticFixtureBase<N> {}
...
#define STATIC_FIXTURE_DEFINE_TEST_NAME_COUNT(N)                               \
  STATIC_FIXTURE_DEFINE_TEST(_##N, N)
...
BENCHMARK_REGISTER_F(DynamicFixture, signed_traditional_indexless)
    ->Arg(MAX_BENCH_SIZE);
BENCHMARK_REGISTER_F(DynamicFixture, unsigned_traditional_indexless)
    ->Arg(MAX_BENCH_SIZE);
BENCHMARK_REGISTER_F(DynamicFixture, alternative_indexless)
    ->Arg(MAX_BENCH_SIZE);
...
#define STATIC_FIXTURE_REGISTER_TEST(NAME)                                     \
  BENCHMARK_REGISTER_F(STATIC_FIXTURE_NAME(NAME), power_indexless)
...
BENCHMARK_REGISTER_F(C_Fixture, search_first)
    ->STATISTICS(REPEATIONS)
    ->Arg(MAX_BENCH_SIZE);
BENCHMARK_REGISTER_F(C_Fixture, search_last)
    ->STATISTICS(REPEATIONS)
    ->Arg(MAX_BENCH_SIZE);
BENCHMARK_REGISTER_F(C_Fixture, fail_first)
    ->STATISTICS(REPEATIONS)
    ->Arg(MAX_BENCH_SIZE);
BENCHMARK_REGISTER_F(C_Fixture, fail_last)
    ->STATISTICS(REPEATIONS)
    ->Arg(MAX_BENCH_SIZE);
