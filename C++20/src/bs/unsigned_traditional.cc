...
export module binary_search.unsigned_traditional;
import binary_search;

//
// Dynamic and static implementations of binary search by the traditional
// algorithm, except array index type is unsigned
//
export {
  ...
  private:
    ...
    [[nodiscard]] inline std::optional<index_t> constexpr twoConditions(
      ...
      while (low < high) {
        ...
        index_t const mid =
            low + (static_cast<index_t>((high - low) + index_t(1)) >> 1u);
        if (data[mid] > v) {
          high = mid - index_t(1);
        } else if (data[mid] < v) {
          low = mid + index_t(1);
      ...
      return (low < data.size()) && (data[low] == v) ? std::make_optional(low)
                                                     : std::nullopt;
    }
