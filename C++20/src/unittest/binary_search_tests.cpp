...
protected:
  void SetUp() override { testdata_ = filler<DataType, IndexType>(GetParam()); }
...
static std::size_t constexpr set5[] = {0u,   1u,   2u,   3u,   10u,
                                       100u, 511u, 512u, 513u, 1u << 15u};
INSTANTIATE_TEST_SUITE_P(ArraySizes, BinarySearchDynamicTests,
                         ::testing::ValuesIn(set5));
...
protected:
  ...
  template <std::size_t N> bool constexpr test() {
    std::array<DataType, N> constexpr testdata2_ =
        filler2<DataType, IndexType, N>();
...
protected:
  void SetUp() override { timeout_timer_ = nullptr; }
...
private:
  void createTimeoutTimer() {
    ...
    struct sigevent kill_signal_after_timeout {
      .sigev_value = {.sival_ptr = &timeout_timer_}, .sigev_signo = SIGKILL,
      .sigev_notify = SIGEV_SIGNAL, ._sigev_un = {
        0
      }
    };
    ASSERT_EQ(0, timer_create(CLOCK_REALTIME, &kill_signal_after_timeout,
                              &timeout_timer_));
  }
  ...
  void startTimeoutTimer() {
    ...
    itimerspec constexpr timeout_duration{
        .it_interval = {0, 0}, .it_value = {DURATION_S, DURATION_NS}};
...
TEST_F(TimeoutDeathTests, signed_traditionalShallTimeout) {
  ...
  enum : std::size_t {
    SIZE =
        static_cast<std::size_t>(std::numeric_limits<SmallIndexType>::max()) +
        1u
  };
  ...
  ASSERT_DEATH(
      {
        ...
        bool const result =
            test(signed_traditional<DataType, SmallIndexType>{}, data);
        ASSERT_TRUE(result);
      },
      "");
}
...
TEST_F(TimeoutDeathTests, unsigned_traditionalShallTimeout) {
  enum : std::size_t { SIZE = std::numeric_limits<SmallIndexType>::max() + 1u };
  ...
  ASSERT_DEATH(
      {
        ...
        bool const result =
            test(unsigned_traditional<DataType, SmallIndexType>{}, data);
        ASSERT_TRUE(result);
      },
      "");
}
...
TEST_F(ProveAsCorrectTests, static_powerPasses) {
  enum : std::size_t { SIZE = std::numeric_limits<SmallIndexType>::max() + 1u };
  static_assert(SIZE == 256u, "");
  std::array<DataType, SIZE> const data =
      filler2<DataType, SmallIndexType, SIZE>();
